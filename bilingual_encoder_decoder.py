...
class EncoderDecoderModel:
    ...
    def __make_model(self):
        self.__model = wrapper.make_model(
            ...
            # decoder(src)
            w_pr = functions.Linear(self.__n_hidden, 4 * self.__n_hidden),
            w_rk = functions.Linear(self.__n_hidden, self.__n_src_embed),
            w_kz = functions.Linear(self.__n_src_embed, len(self.__src_vocab)),
            w_zr = functions.EmbedID(len(self.__src_vocab), 4 * self.__n_hidden),
            w_rr = functions.Linear(self.__n_hidden, 4 * self.__n_hidden),
        )
    ...
    def save(self, fp):
        ...
        print_linear(self.__model.w_pr)
        print_linear(self.__model.w_rk)
        print_linear(self.__model.w_kz)
        print_embed(self.__model.w_zr)
        print_linear(self.__model.w_rr)
    ...
    def load(line_gen):
        ...
        copy_linear(self.__model.w_pr)
        copy_linear(self.__model.w_rk)
        copy_linear(self.__model.w_kz)
        copy_embed(self.__model.w_zr)
        copy_linear(self.__model.w_rr)
    ...
    def __predict(self, is_training, src_batch, trg_batch = None, generation_limit = None):
        ...
        src_itos = self.__src_vocab.itos
        ...
        s_d, s_q = lstm(s_c, m.w_pq(s_p))
        s_e, s_r = lstm(s_c, m.w_pr(s_p))

        htrg_batch = [[] for _ in range(batch_size)]
        hsrc_batch = [[] for _ in range(batch_size)]
        ...
        if is_training:
            ...
            for l in range(trg_len):
                ...
                for k in range(batch_size):
                    htrg_batch[k].append(trg_itos(output[k]))

                s_d, s_q = lstm(s_d, m.w_yq(s_t) + m.w_qq(s_q))

            for l in range(src_len):
                s_k = tanh(m.w_rk(s_r))
                r_z = m.w_kz(s_k)

                s_u = wrapper.make_var([src_stoi(src_batch[k][l]) for k in range(batch_size)], dtype=np.int32)
                accum_loss += functions.softmax_cross_entropy(r_z, s_u)
                
                output = wrapper.get_data(r_z).argmax(1)
                for k in range(batch_size):
                    hsrc_batch[k].append(src_itos(output[k]))

                s_e, s_r = lstm(s_e, m.w_zr(s_u) + m.w_rr(s_r))

            return htrg_batch, hsrc_batch, accum_loss
        else:
            while len(htrg_batch[0]) < generation_limit:
                s_j = tanh(m.w_qj(s_q))
                r_y = m.w_jy(s_j)
                ...
                if all(htrg_batch[k][-1] == '</s>' for k in range(batch_size)): break
    ...
    def train(self, src_batch, trg_batch):
        ...
        htrg_batch, hsrc_batch, accum_loss = self.__predict(True, src_batch, trg_batch=trg_batch)
        
        accum_loss.backward()
        self.__opt.clip_grads(10)
        self.__opt.update()

        return htrg_batch, hsrc_batch, wrapper.get_data(accum_loss).reshape(())
...
def train_model(args):
    ...
    for epoch in range(args.epoch):
        ...
        for src_batch, trg_batch in generate_train_minibatch(args.source, args.target, args.minibatch):
            htrg_batch, hsrc_batch, loss = model.train(src_batch, trg_batch)
            K = len(src_batch)

            for k in range(K):
                trace('epoch %3d/%3d, sample %8d' % (epoch + 1, args.epoch, trained + k + 1))
                trace('  src = ' + ' '.join([x if x != '</s>' else '*' for x in src_batch[k]]))
                trace('  hsrc= ' + ' '.join([x if x != '</s>' else '*' for x in hsrc_batch[k]]))
                trace('  trg = ' + ' '.join([x if x != '</s>' else '*' for x in trg_batch[k]]))
                trace('  htrg= ' + ' '.join([x if x != '</s>' else '*' for x in htrg_batch[k]]))
